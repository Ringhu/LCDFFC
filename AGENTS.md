...
2. **研究文档维护约定**：如何使用和更新 `README.md`、`INSTRUCTION.md`、`CLAUDE.md`、其他研究文档。
...
- `AGENTS.md`：当前仓库的实际工作规则和文档同步规则。
- `chat.md`：研究意图、论文路线、阶段验收标准的来源文档。
- `INSTRUCTION.md`：实验推进顺序、Sprint 任务、止损点。
- `CLAUDE.md`：工程结构、接口、编码规范、模块边界。
- `README.md`：对外概览、快速开始、当前可运行入口。

如果 `README.md`、`INSTRUCTION.md`、`CLAUDE.md` 与实际代码不一致，应以**代码事实 + `agents.md` 更新规则**为准，随后立刻修正文档。
...
- 提供阶段性验收标准，例如：
  - 先做 `CityLearn`
  - 先做 `central_agent=True`
  - 先做 battery-only control
  - 先做 fixed-weight `forecast + QP/MPC`
  - 先证明闭环优于或至少打平 `RBC`

规则：

- `chat.md` 是**研究意图参考文档**，不是日常进度日志。
- **不要把开发进度回写到 `chat.md`**。
...
- 描述工程结构、模块职责、接口约束和编码约定。
- 适合回答“应该怎么组织代码”“哪些模块能互相依赖”。

规则：

- 当模块边界、公共接口、配置范式、关键编码规范发生变化时，更新 `CLAUDE.md`。
- 它记录的是**稳定工程约定**，不记录细碎实验结果。
...
- 作为实验执行手册和 Sprint 管理文档。
- 适合回答“现在应该做什么”“这一步的止损点是什么”“接下来按什么顺序推进”。
...
- 它记录的是**执行计划与阶段状态**，不是对外介绍。

### `README.md`

用途：

- 对外说明项目是什么、当前能做什么、如何运行。
- 面向首次进入仓库的读者。

规则：

- 当入口命令、依赖、目录结构、当前可运行功能发生变化时更新。
- `README.md` 只能写**当前真实可运行**的内容。
